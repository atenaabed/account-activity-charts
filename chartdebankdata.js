...
async function dataChart(){
    try {
      ...
      // Swap transaction((execute)) Chart
       new Chart(
        ...
        //Get Sends and Receives data of Swap transaction((swap)) Chart
        const transactionSends = responseData['swap transaction((swap))'].transactions
        const transactionSends2 = transactionSends[0]

        //listOfSwapTRansaction
        const SwapTransactionData = []
 
        // get Sends Value
        const sends = transactionSends2.sends
 
       // get Receives Value
        const receives = transactionSends2.receives
        SwapTransactionData.push(sends[0], receives[0])


        //Swap transaction((swap)) Chart
        new Chart(
          document.getElementById('SwapTransaction(execute)'),
          {
            type: 'bar',
            data: {
              labels: ['Sends','Receives'],
              datasets: [
                {
                  label: 'swap transaction((swap))',
                  data: SwapTransactionData.map(row => row.value)
        ...
        // Get Value for calculating profit of SwapTransactionSwap --> this data is related to above data
        const profitSwap = []

        //get Value of Sends 
        const SendsValue = sends[0]
        const SendsValue2 = SendsValue.value

        // get Value of Receives
        const ReceiveValue = receives[0]
        const ReceiveValue2 = ReceiveValue.value

        profitSwap.push(ReceiveValue2 - SendsValue2)


       //SwapTransactionSwap - Profit Chart
        new Chart(
          document.getElementById('SwapTransaction(swap)-Profit'),
          {
            type: 'bar',
            data: {
              labels: ['Profit'],
              datasets: [
                {
                  label: 'swap transaction((swap))-Profit',
                  data: profitSwap
    ...
    function createBarGraphData(responseData) {
      ...
      // Get yearly data for yearly Chart  
       const getYearly = barGraphData.yearly
       const secondValue = Object.values(getYearly)
       const thirdValue = Object.values(secondValue)
       const result = thirdValue.map(item =>Object.keys(item))
       
       //Get name of whole transaction in each year
       const resultdata = result[0]
      
      //Calculate profit of each transaction
       const profit = []
       thirdValue.forEach(item =>{
        const valueOfSendsandRecives = Object.values(item)
        valueOfSendsandRecives.forEach(option =>{
            const dataSendsandRecives = Object.values(option)
            profit.push(dataSendsandRecives[1] - dataSendsandRecives[0])
        })
       ...
       //Create yearly chart according to profits
       new Chart(
